...
{
    public partial class 取款a : UserControl
    {
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                DialogResult result = MessageBox.Show("请确认是否从账户 " + textBox5.Text + " 内取出金额 " + textBox2.Text + " (CNY) 。", "确认保存", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    
                    //取款过程
                    //MessageBox.Show("取款金额 "+textBox2.Text + " (CNY) 大于账户内余额 "+textBox1.Text+" (CNY)!");
                    MessageBox.Show("成功从账户 " + textBox5.Text + " 内取出金额 " + textBox2.Text + " (CNY) ！");
                    textBox2.Text = "";//成功或失败后清空
